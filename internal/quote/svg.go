...
func (SVGBuilder) Build(card CardLayout) ([]byte, error) {
	...
	out.WriteString(`</defs>`)
	for index, row := range card.Rows {
		writeRow(&out, index, row, card.Theme, card.FontFamily)
	}
	out.WriteString(`</svg>`)
...
func writeText(out *bytes.Buffer, x, y, size float64, color, fontFamily, value string) {
	fmt.Fprintf(out, `<text x="%s" y="%s" font-family="%s" font-size="%s" fill="%s">`, px(x), px(y), fontFamily, px(size), color)
